...
if (isset($_POST['search'])) {
    $search = "%$_POST[search]%";
    $sql = "SELECT account_nb, firstname, lastname FROM user WHERE firstname LIKE :search OR lastname LIKE :search AND ID != :id AND active = 1 LIMIT 100";
    ...
    $sql = "SELECT account_nb, firstname, lastname FROM user WHERE ID != :id AND active = 1 LIMIT 100";
    $query = $dbh->prepare($sql);
    $query->bindParam(":id", $id, PDO::PARAM_INT);
    $query->execute();
