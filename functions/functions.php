...
function isGranted($redirect = true, $role = ["ROLE_USER"])
{
    global $dbh;

    if (isset($_SESSION['user-id'])) {
        $id = intval($_SESSION['user-id']);
        $sql = "SELECT email, role FROM user WHERE ID = :id";
        $query = $dbh->prepare($sql);
        $query->bindParam(":id", $id, PDO::PARAM_INT);
        $query->execute();
    ...
    $roles .= "pour accéder à la page";

    if($redirect) {
        return redirectNotification("Vous devez être connecté avec un role : $roles", getRoute("home"), "danger");
    } 
    return false;
}
