...
if($user["can_pay"] == 0) {
    redirectNotification("Vous n'êtes pas autorisé à faire des virements !", getRoute("home"), "danger");
}
...
if(isset($_POST['transfer'])){
    if(!isset($_POST['amount']) || empty($_POST['amount']) || !isset($_POST['to']) || empty($_POST['to'])){
        ...
        $sql = "SELECT balance FROM account WHERE user = :id";
...
function makeTransfer($user, $to, $amount){
    ...
    if(!getUser($to)){
        redirectNotification("Cet utilisateur n'existe pas !", "", "danger");
        die();
    } else if (!getUserAccount($to)) {
        redirectNotification("Le compte spécifié existe en tant qu'utilisateur mais n'as pas de compte bancaire associé !", "", "danger");
        die();
